...
app.config['PUBLIC_KEY'] = os.environ.get('STRIPE_PUBLISHABLE_KEY')
app.config['SECRET_KEY'] = os.environ.get('STRIPE_SECRET_KEY')
...
def pay():
    ...
    if user.paid == 0:
    	return render_template('user/buy.html', key=app.config['PUBLIC_KEY'], email=current_user.email)
...
def charge():
    ...
    flash('Thank you for your payment!', 'positive')
